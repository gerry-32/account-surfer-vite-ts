...
const ToggleDrag = () => {
  const [, storeShowHidden] = useStoreChange('showHidden')
  const [dragEnabled, storeDragEnabled] = useStoreChange('dragEnabled')
  return (
    <div
      className={`${
      ...
      onClick={() => {
        if (!dragEnabled) storeShowHidden(false)
        storeDragEnabled(!dragEnabled)
      }}
    >
      <span
        className={`${
